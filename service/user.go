...
//checkUser checks weher user is valid
func checkUser(username, password string) bool {
    es_client, err := elastic.NewClient(elastic.SetURL(ES_URL), elastic.SetSniff(false))
    if err != nil {
        fmt.Printf("ES is no setup %v\n", err)
        return false
    }
...
//addUser adds a new user, return true if successfully
...
//if login is successful, a token is created
func loginHandler(w http.ResponseWriter, r *http.Request) {
    fmt.Println("Receive one login request")
